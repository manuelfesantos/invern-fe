...
const Footer = () => {
    ...
    const search = async () => {
        if(ref.current && ref.current.value) {
            const response = await fetch(`https://preview.invern-be.pages.dev/products?search=${ref.current.value}`, {
                headers: {
    ...
    <>
        {
            show && (
                <div className='absolute top-0 left-0 z-30 h-screen w-screen flex items-center justify-center bg-black bg-opacity-85'>
                    ...
                    <div className='absolute top-5 right-5 z-30 cursor-pointer' onClick={() => {
                        setShow(false)
                        setProducts([])
                    }}>
        ...
        <footer className='flex flex-col'>
            ...
            <form className='z-40'>
                <div className="relative h-10">
                    ...
                    <input type="search" ref={ref} id="search" className="h-full w-screen pl-14 bg-opacity-70 bg-[#4C4B48] focus:outline-none" placeholder="search..." required autoComplete="off"/>
                    <CustomButton position="absolute h-full end-0 px-6" type='button' onClick={search}>search</CustomButton>
                </div>
            </form>
        </footer>
    </>
