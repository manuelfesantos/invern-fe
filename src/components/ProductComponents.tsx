...
const ProductComponents = ({children,product,component}:{children:ReactNode,product:IProduct,component:string}) => {

  const {cart,setCart} = useContext<CartContext>(cartContext as Context<CartContext>);
  const {wishList,setWishList} = useContext<WishListContext>(wishListContext as Context<WishListContext>);
  ...
  const toggleLike = () => {
    if(wishList.products.find(item => item.productId === product.productId)) {
      setWishList((prevWishList:WishList) => ({
        products:prevWishList.products.filter(item => item.productId !== product.productId)
      }));
    ...
    return (
      <>
        ...
        </div>
        {
          product.stock === 0
            ? (
              <div className='py-4'><h4 className='font-extrabold text-red-400'>Sold Out</h4></div>
            )
            : (
              <CustomButton
